...
//  WeedConnectMobile
//
//  Created by Javier J Calderon Jr on 1/19/19.
...
class APIManager {
    ...
    // API - Getting Dispensarys List
    func getDispensarys(completionHandler: @escaping (JSON) -> Void) {
        
        let path = "api/customer/dispensarys/"
        requestServer(.get, path, nil, URLEncoding(), completionHandler)
        
    }
    
    // API - Getting list of Products of Dispensary.
        ...
        if JSONSerialization.isValidJSONObject(jsonArray) {
            ...
            do {
                ...
                let params: [String: Any] = [
                    ...
                    "dispensary_id": "\(Tray.currentTray.dispensary!.id!)",
    ...
    func pickOrder(orderId: Int, completionHandler: @escaping (JSON) -> Void) {
        let path = "api/driver/oreder/pick/"
        let params: [String: Any] = [
            "order_id": "\(orderId)",
            "access_token": self.accessToken
        ]
